...
export const ProjectCard = ({ project }: ProjectCardProps) => {
  return (
    <div className="group flex h-full flex-col overflow-hidden rounded-xl border border-neutral-800 bg-neutral-900 transition hover:border-neutral-700">
      <div className="relative h-40 w-full overflow-hidden">
        ...
        <div className="absolute inset-0 hidden items-center justify-center gap-6 bg-black/60 opacity-0 transition group-hover:opacity-100 md:flex">
          {project.source.map((source, i) => (
            <a
              key={i}
              href={source.href}
              target="_blank"
        ...
        <div className="mt-auto flex gap-2 md:hidden">
          {project.source.map((source, i) => (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-1 rounded-md bg-white px-2 py-1 text-xs text-black"
            >
              {source.type === "website" ? (
                <>
                ...
                <>
                  <GithubIcon color="black" width={12} height={12} />
                  Code
                </>
              )}
            </a>
